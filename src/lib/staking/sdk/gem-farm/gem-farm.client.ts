...
export class GemFarmClient extends GemBankClient {
  ...
  async initFarmer(
    ...
    const ix = await this.farmProgram.instruction.initFarmer({
      accounts: {
        farm,
        farmer,
        identity: identityPk,
  ...
  async stakeCommon(
    ...
    let ix;
    if (unstake) {
      ix = await this.farmProgram.instruction.unstake(
        farmAuthBump,
        farmTreasuryBump,
        farmerBump,
        skipRewards,
      ...
      ix = await this.farmProgram.instruction.stake(farmAuthBump, farmerBump, {
        accounts: {
          farm,
          farmer,
          identity: identityPk,
  ...
  async claim(
    ...
    const ix = await this.farmProgram.instruction.claim(
      farmAuthBump,
      farmerBump,
      potABump,
      potBBump,
